...
public class UIGrid : UIWidgetContainer
{
    ...
    public class ST_GridItem
    {
        public int id;
        public GameObject ele;
    }
    protected List<ST_GridItem> m_listGrid = new List<ST_GridItem>();
    ...
    public ST_GridItem GetItemByIdx(int nIdx)
    {
        return m_listGrid[nIdx];
    }

    public int GetIdxById(int id)
    {
        int nRes = -1;
        for (int nIdx = 0; nIdx < m_listGrid.Count; nIdx++)
        {
            if (m_listGrid[nIdx].id == id)
            {
                nRes = nIdx;
                break;
            }
	...
	protected void ResetPosition (List<Transform> list)
	{
        ...
        {
            // Re-add the children in the same order we have them in and position them accordingly
            for (int i = 0, imax = m_listGrid.Count; i < imax; ++i)
            {
                Transform t = m_listGrid[i].ele.transform;
                // See above
                //t.parent = myTrans;
